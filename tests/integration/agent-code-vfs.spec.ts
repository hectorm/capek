...
test.describe("Agent Code VFS", () => {
  test("should interact with VFS via code interpreter", async ({ page }) => {
    ...
    const testMessage = "Write a file to the vfs workspace and read it back.";
    ...
    // Verify the VFS workspace file was written and read back
    await expect(response.filter({ hasText: "VFS test content" })).toBeVisible({ timeout: 15000 });

    // Verify the MCP server is listed
    await expect(response.filter({ hasText: "Mock_MCP" })).toBeVisible({ timeout: 5000 });

    // Verify the MCP tool was executed
    await expect(response.filter({ hasText: "Mock search result" })).toBeVisible({ timeout: 5000 });
